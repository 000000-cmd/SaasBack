...
@Repository
public interface JpaFlowControlRepository extends JpaRepository<FlowControlEntity, UUID> {
    List<FlowControlEntity> findBySectionIdInOrderByDisplayOrderAsc(Collection<UUID> sectionIds);
    List<FlowControlEntity> findBySectionIdOrderByDisplayOrderAsc(UUID sectionId);
     ...
     */
    @Modifying
    @Query(value = "UPDATE flow_control SET Visible = 1, Enabled = 1, Label = :label, "
